  ...
  <div class="landlord-orders-page">
    <div class="container">
      <div class="page-header">
        <h1>我的租客订单</h1>
        <p class="text-secondary">查看和管理您的租客订单</p>
      </div>
        ...
        <div v-for="order in orders" :key="order.orderNo" class="order-item card">
          ...
          <div class="order-content" @click="viewDetail(order.orderNo)">
            <div class="house-info">
              ...
              <div class="tenant-info mt-sm">
                👤 租客ID: {{ order.tenantId }} <span v-if="order.tenantName">({{ order.tenantName }})</span>
              </div>
              <div class="rent-period mt-xs">
            ...
            <div class="order-amount">
              ...
              <span v-if="order.orderStatus === 'PAID' || order.orderStatus === 'COMPLETED'" class="text-success text-sm mt-xs">
                预计收入: ¥{{ order.totalAmount }}
              </span>
            </div>
          </div>
          ...
          <div class="order-footer">
            ...
            <div class="order-actions">
              ...
              <button class="btn btn-primary btn-sm" @click="contactTenant(order.tenantId)">联系租客</button>
            </div>
          </div>
        </div>
      </div>
...
import { getLandlordOrders } from '../../api/order'
  ...
  setup() {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        const res = await getLandlordOrders(params)
    ...
    // 联系租客
    const contactTenant = (tenantId) => {
      // 如果实现了消息系统，可以跳转到聊天页面
      router.push({
         path: '/notifications',
         query: { chatWith: tenantId }
      })
    }
